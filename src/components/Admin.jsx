...
function Admin({ inventory, setInventory }) {
  ...
  if (!isLoggedIn) {
    return (
      <div className="screen-container" style={{ padding: '2rem 1rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ background: '#f5f5f5', padding: '2rem', borderRadius: '12px', width: '100%', maxWidth: '400px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
  ...
  return (
    <div className="screen-container" style={{ padding: '1rem', paddingBottom: '80px', overflowY: 'auto' }}>
      ...
      {isAdding && (
        <div style={{ background: '#f5f5f5', padding: '1rem', borderRadius: '8px', marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '1rem' }}>
            ...
            <button className="cart-btn" style={{ background: '#ccc', color: '#333' }} onClick={() => setIsAdding(false)}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div>
        {inventory.map(item => (
          <div key={item.id} style={{ border: '1px solid #ddd', padding: '1rem', borderRadius: '8px', marginBottom: '1rem', background: '#fff' }}>
            {editingId === item.id ? (
              <div>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    <p style={{ margin: '0 0 0.5rem 0', color: '#666' }}>{item.size} • Rs {item.price}</p>
